window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// dsh 0.1.2 把会话聊天数据从 `useSession().nodes`（数组）挪到了
		// `useChat()` 的 ChatSnapshot 里：新版 `nodes` 是 Map/Store，同时为了过渡
		// 保留了 `legacy.nodes`、`legacy.partial`、`legacy.turnTimings`。所有读会话
		// 聊天数据的路径都先归一化，兼容 0.1.1 与 0.1.2 两种结构。
		function chatNodesOf(snapshot) {
			if (!snapshot) return [];
			...
			if (snapshot.nodes && typeof snapshot.nodes.values === "function") {
				return Array.from(snapshot.nodes.values());
			}
			return [];
		}
		...
		function findAssistantNode(snapshot, seq) {
			for (const node of chatNodesOf(snapshot)) {
				if (node && node.kind === "assistant" && node.seq === seq) return node;
			}
			return void 0;
		}
		...
		const zh = {
			...
			"balance.reset.confirm": "清零「本次打开」和「{period}」的费用与用量？不可撤销。",
			...
			"balance.weekly.title": "本周费用",
			"balance.monthly.title": "本月费用",
			"balance.price.title": "目前单价",
			"balance.price.title_nocurrency": "目前单价",
			...
			"balance.price.unit_note": "DeepSeek 已按{period}折算；其他供应商为基准价",
		...
		const en = {
			...
			"balance.reset.confirm": "Clear cost and usage for this session and {period}? This cannot be undone.",
			...
			"balance.weekly.title": "This week's cost",
			"balance.monthly.title": "This month's cost",
			"balance.price.title": "Current price",
			"balance.price.title_nocurrency": "Current price",
			...
			"balance.price.unit_note": "DeepSeek prices reflect {period} rates; other providers are base prices",
		...
		const css = ""
			...
			+ ".dsbPanelBody .dsbSection > .dsbRow,.dsbPanelBody .dsbSection > .dsbNote,.dsbPanelBody .dsbSection > .dsbModelBlock,.dsbPanelBody .dsbSection > .dsbCostTable,.dsbPanelBody .dsbSection > .dsbPriceTable,.dsbPanelBody .dsbSection > .dsbPeriodTabs{margin-left:16px}"
			...
			// 重置按钮：钉在面板标题栏右侧、关闭按钮左边。它清的是「本次打开 + 当前
			// 选中周期」两笔，横跨了下面好几节内容，所以放在标题栏这个面板级的位置，
			// 而不是塞进某一节里——塞进用量那一节会让人以为只清用量、不清费用。
			//
		...
		// 周费用：周一 00:00 到周日 23:59:59 累计，跨周自动清零。
		const WEEKLY_STORE_KEY = "dsh-ui-balance/weeklyCostStore/v1";
		function readStoredWeekly() {
			try {
				if (typeof localStorage === "undefined") return null;
				const raw = localStorage.getItem(WEEKLY_STORE_KEY);
				return raw === null ? null : JSON.parse(raw);
			} catch {
				return null;
			}
		}
		function writeStoredWeekly(value) {
			try {
				if (typeof localStorage === "undefined") return;
				localStorage.setItem(WEEKLY_STORE_KEY, JSON.stringify(value));
			} catch {
				// 存储不可用时，本次会话内仍可累计。
			}
		}
		function weekKeyOf(date) {
			const d = date instanceof Date ? date : new Date(date);
			const day = d.getDay();
			const offset = (day + 6) % 7; // 周一为每周第一天
			const monday = new Date(d.getFullYear(), d.getMonth(), d.getDate() - offset);
			return `${monday.getFullYear()}-${String(monday.getMonth() + 1).padStart(2, "0")}-${String(monday.getDate()).padStart(2, "0")}`;
		}
		function resetWeekly() {
			return {
				week: weekKeyOf(new Date()),
				currency: null,
				totalCost: 0,
				perModel: new Map(),
				accounted: new Map()
		...
		function readWeekly() {
			const stored = readStoredWeekly();
			const current = weekKeyOf(new Date());
			if (stored?.week !== current) return resetWeekly();
			return {
				week: current,
				currency: stored.currency ?? null,
				totalCost: Number(stored.totalCost ?? 0),
		...
		// localStorage 按 origin（含端口）隔离，桌面端每次启动端口可能不同，所以
		// 本日/周/月度费用还要通过 host 半的文件路由跨 origin 持久化。
		function writePeriodStoreRemote(dailyState, weeklyState, monthlyState) {
			if (typeof fetch === "undefined") return;
			try {
				fetch("/api/dsdesktop/balance/cost-store", {
					method: "POST",
					headers: { "content-type": "application/json" },
					body: JSON.stringify({ daily: dailyState, weekly: weeklyState, monthly: monthlyState }),
					keepalive: true
				}).catch(() => {
					// 文件写入失败时，本次会话内仍可继续累计，下次启动会回退到 localStorage。
				});
			} catch {
				// 同上。
			}
		...
		const costStore = (() => {
			...
			let daily = readDaily();
			// 周费用累计器：周一 00:00 - 周日 23:59:59，跨启动保留（localStorage）。
			...
			// 本次打开里被手动清零过的周期：挡住启动时那次异步回填把旧数字灌回来。
			const resetPeriods = new Set();
			let rolloverTimer = null;
			let snapshot = freeze();

			function freeze() {
				if (daily.day !== dayKeyOf(new Date())) daily = resetDaily();
				if (weekly.week !== weekKeyOf(new Date())) weekly = resetWeekly();
				...
				return Object.freeze({
					...
					weekly: Object.freeze({
						...
						perModel: Object.freeze(
							Array.from(weekly.perModel.values()).map((m) => Object.freeze({ ...m, tokens: Object.freeze({ ...m.tokens }) }))
						)
					}),
			...
			function persist() {
				...
				const weeklyState = {
					...
					currency: weekly.currency,
					totalCost: weekly.totalCost,
					perModel: Array.from(weekly.perModel.entries()),
					accounted: Array.from(weekly.accounted.entries())
				};
				const monthlyState = {
					month: monthly.month,
					currency: monthly.currency,
				...
				writeStoredDaily(dailyState);
				writeStoredWeekly(weeklyState);
				writeStoredMonthly(monthlyState);
				writePeriodStoreRemote(dailyState, weeklyState, monthlyState);
			}
			function notify() {
				snapshot = freeze();
				persist();
			...
			function tokenDeltaOf(usage) {
				return {
					// dsh 的 TokenUsage 约定：inputTokens 已经是「未命中缓存的输入」；
					// 若 provider 还单独上报 cacheWriteTokens，计费按 miss 价，展示/命中率也按 miss 归并。
			...
			function monthlyAccountMessage(accountKey, turnStartTime, usage, selection, pricing) {
				...
				// 只有 DeepSeek 官方 provider 适用峰谷倍率；其他厂家按基准价计算。
				const peak = selection && isDeepSeekProvider(selection.provider) ? isPeakHours(new Date(turnStartTime)) : false;
			...
			function weeklyApplyDelta(key, tokens, cost, selection, priced) {
				const entry = weekly.perModel.get(key) ?? {
					provider: selection?.provider ?? null,
					model: selection?.model ?? null,
					priced,
					tokens: { input: 0, cacheRead: 0, output: 0 },
				...
				weekly.perModel.set(key, entry);
			}

			function weeklyRemoveDelta(key, tokens, cost) {
				const entry = weekly.perModel.get(key);
				if (!entry) return;
				entry.tokens.input -= tokens.input;
				entry.tokens.cacheRead -= tokens.cacheRead;
				...
				entry.cost -= cost;
				if (entry.tokens.input <= 0 && entry.tokens.cacheRead <= 0 && entry.tokens.output <= 0 && entry.cost <= 0) {
					weekly.perModel.delete(key);
				}
			}

			/** 把一条最终 assistant 消息计入「本周费用」，周一 00:00 - 周日 23:59:59。 */
			function weeklyAccountMessage(accountKey, turnStartTime, usage, selection, pricing) {
				if (turnStartTime === void 0 || usage === void 0 || usage === null) return false;
				const now = new Date();
				const current = weekKeyOf(now);
				if (weekly.week !== current) weekly = resetWeekly();
				if (weekKeyOf(turnStartTime) !== current) return false;
				if (weekly.currency === null && pricing) weekly.currency = pricing.currency;
				const key = priceKey(selection);
				const priceEntry = pricing && selection ? pricing.modelPricing?.[key] : void 0;
				const peak = selection && isDeepSeekProvider(selection.provider) ? isPeakHours(new Date(turnStartTime)) : false;
				const cost = costOfUsage(usage, priceEntry, pricing?.peakMultiplier, peak);
				...
				const prior = weekly.accounted.get(accountKey);
				if (prior) {
					if (prior.priced) return false;
					if (prior.key === key && !priced) return false;
					weeklyRemoveDelta(prior.key, prior.tokens, prior.cost);
					weekly.totalCost -= prior.cost;
				}
				weekly.accounted.set(accountKey, next);
				weeklyApplyDelta(key, tokens, cost, selection, priced);
				return true;
			}
			...
			function applyPeriodEstimate(liveKey, item, modelKey, tokens, cost) {
				if (item.startTime === void 0) return;
				const now = new Date();
				if (dayKeyOf(item.startTime) === dayKeyOf(now)) {
					if (daily.day !== dayKeyOf(now)) daily = resetDaily();
					if (daily.currency === null && currency) daily.currency = currency;
					dailyApplyDelta(modelKey, tokens, cost, { provider: item.provider, model: item.model }, item.priced === true);
					daily.accounted.set(liveKey, { key: modelKey, priced: item.priced === true, cost, tokens });
				}
				if (weekKeyOf(item.startTime) === weekKeyOf(now)) {
					if (weekly.week !== weekKeyOf(now)) weekly = resetWeekly();
					if (weekly.currency === null && currency) weekly.currency = currency;
					weeklyApplyDelta(modelKey, tokens, cost, { provider: item.provider, model: item.model }, item.priced === true);
					weekly.accounted.set(liveKey, { key: modelKey, priced: item.priced === true, cost, tokens });
				}
				if (monthKeyOf(item.startTime) === monthKeyOf(now)) {
					if (monthly.month !== monthKeyOf(now)) monthly = resetMonthly();
					if (monthly.currency === null && currency) monthly.currency = currency;
					monthlyApplyDelta(modelKey, tokens, cost, { provider: item.provider, model: item.model }, item.priced === true);
					monthly.accounted.set(liveKey, { key: modelKey, priced: item.priced === true, cost, tokens });
				}
			}

			function removePeriodEstimate(liveKey, estimated) {
				if (daily.accounted.has(liveKey)) {
					dailyRemoveDelta(estimated.key, estimated.tokens, estimated.cost);
					daily.totalCost -= estimated.cost;
					daily.accounted.delete(liveKey);
				}
				if (weekly.accounted.has(liveKey)) {
					weeklyRemoveDelta(estimated.key, estimated.tokens, estimated.cost);
					weekly.totalCost -= estimated.cost;
					weekly.accounted.delete(liveKey);
				}
				if (monthly.accounted.has(liveKey)) {
					monthlyRemoveDelta(estimated.key, estimated.tokens, estimated.cost);
					monthly.totalCost -= estimated.cost;
					monthly.accounted.delete(liveKey);
				}
			}
			...
			function commitLiveNow(sessionId) {
				...
				for (const key of keys) {
					...
					applyPeriodEstimate(key, item, modelKey, tokens, cost);
					live.delete(key);
				}
			}
			...
			function noteLiveEstimate({ sessionId, turn, step, tokens, cost, selection, priced, currency: currencyCode, startTime }) {
			 ...
			 * 清零一个统计周期的费用与用量。
			 *
			 * `accounted` 里的记录**保留但清零**，不整个丢掉：那张表是「这条消息已经
			 * 计过账」的凭证，丢了的话，面板重开/热重载让探针的 effect 重跑一遍，被
			 * 清掉的历史消息会重新计进来——用户会看到刚清零的数字自己长回来。改成把
			 * 每条记录的 cost/tokens 归零：既继续挡住重复计费，也让「未计价→计价」
			 * 迁移路径上的 removeDelta 减掉的是 0，不会把累计减成负数。
			 */
			function clearPeriodState(state) {
				state.totalCost = 0;
				state.perModel = new Map();
				for (const [key, prior] of state.accounted) {
					state.accounted.set(key, { ...prior, cost: 0, tokens: { input: 0, cacheRead: 0, output: 0 } });
				}
			}

			/**
			 * 清零「本次打开」那一份累计。跟周期一样，`accounted` / `estimatedLive`
			 * 里的记录保留但归零：留着挡重复计费，归零保证后续的撤销/迁移路径减掉的
			 * 是 0，不会把刚清零的总额减成负数。进行中的流式估算（live）不动——那是
			 * 正在发生的消费，清零清的是过去。
			 */
			function clearSessionState() {
				totalCost = 0;
				perModel.clear();
				for (const [key, prior] of accounted) {
					accounted.set(key, { ...prior, cost: 0, tokens: { input: 0, cacheRead: 0, output: 0 } });
				}
				for (const [key, prior] of estimatedLive) {
					estimatedLive.set(key, { ...prior, cost: 0, tokens: { input: 0, cacheRead: 0, output: 0 } });
				}
			}

			function rolloverIfNeeded() {
				let changed = false;
				if (daily.day !== dayKeyOf(new Date())) { daily = resetDaily(); changed = true; }
				if (weekly.week !== weekKeyOf(new Date())) { weekly = resetWeekly(); changed = true; }
				if (monthly.month !== monthKeyOf(new Date())) { monthly = resetMonthly(); changed = true; }
				if (changed) notify();
			}
			...
			return {
				...
				/**
				 * 清零指定统计周期（day/week/month）**以及「本次打开」**的费用与用量，
				 * 并落盘（localStorage + host 侧文件），关掉应用再打开也不会长回来。
				 *
				 * 「本次打开」不论选哪个周期都一起清：它在费用汇总表里跟日/周/月并排，
				 * 自己却没有对应的周期选项——只清周期的话，那一格会一直挂着旧数字，
				 * 看上去像是「重置没生效」。
				 */
				resetPeriod(period) {
					const state = period === "week" ? weekly : period === "month" ? monthly : daily;
					clearPeriodState(state);
					clearSessionState();
					// 启动时那次跨 origin 回填是异步的：清零如果正好赶在它返回之前，
					// 回填看到「本地是 0」就会把文件里的旧数字灌回来，界面上表现为
					// 「点了确认，数字过一会儿自己回来了」。记下手动清过的周期，回填
					// 直接跳过它。
					resetPeriods.add(period === "week" ? "week" : period === "month" ? "month" : "day");
					notify();
				},
				subscribe(fn) {
					listeners.add(fn);
				...
				noteLiveEstimate,
				isPricedAccounted(accountKey) {
					const prior = accounted.get(accountKey);
					return prior !== void 0 && prior.priced === true;
				},
				hydrateDailyMonthly(dailyData, weeklyData, monthlyData) {
					let changed = false;
					if (resetPeriods.has("day")) dailyData = null;
					if (resetPeriods.has("week")) weeklyData = null;
					if (resetPeriods.has("month")) monthlyData = null;
					if (dailyData && daily.totalCost === 0 && daily.perModel.size === 0 && dailyData.day === daily.day) {
						daily = {
							day: daily.day,
							currency: dailyData.currency ?? daily.currency,
							totalCost: Number(dailyData.totalCost ?? 0),
							perModel: new Map(Array.isArray(dailyData.perModel) ? dailyData.perModel : []),
							accounted: new Map(Array.isArray(dailyData.accounted) ? dailyData.accounted : [])
						};
						changed = true;
					}
					if (weeklyData && weekly.totalCost === 0 && weekly.perModel.size === 0 && weeklyData.week === weekly.week) {
						weekly = {
							week: weekly.week,
							currency: weeklyData.currency ?? weekly.currency,
							totalCost: Number(weeklyData.totalCost ?? 0),
							perModel: new Map(Array.isArray(weeklyData.perModel) ? weeklyData.perModel : []),
							accounted: new Map(Array.isArray(weeklyData.accounted) ? weeklyData.accounted : [])
						};
						changed = true;
					}
					if (monthlyData && monthly.totalCost === 0 && monthly.perModel.size === 0 && monthlyData.month === monthly.month) {
						monthly = {
							month: monthly.month,
							currency: monthlyData.currency ?? monthly.currency,
							totalCost: Number(monthlyData.totalCost ?? 0),
							perModel: new Map(Array.isArray(monthlyData.perModel) ? monthlyData.perModel : []),
							accounted: new Map(Array.isArray(monthlyData.accounted) ? monthlyData.accounted : [])
						};
						changed = true;
					}
					if (changed) notify();
				...
				noteMessage(sessionId, accountKey, turnStartTime, usage, selection, pricing, turnId, step) {
					...
					if (liveKey !== void 0) {
						...
						if (estimated) {
							...
							removePeriodEstimate(liveKey, estimated);
							estimatedLive.delete(liveKey);
							touched = true;
						}
					}
					// 本日/周/月度费用不受「本次打开」限制：当天/当周/当月内消息哪怕是翻旧会话翻出来的也要计。
					if (dailyAccountMessage(accountKey, turnStartTime, usage, selection, pricing)) touched = true;
					if (weeklyAccountMessage(accountKey, turnStartTime, usage, selection, pricing)) touched = true;
		...
		// 跨 origin 持久化：localStorage 只对当前端口有效，启动后异步从 host 文件读回本日/月度费用。
		if (typeof fetch !== "undefined") {
			fetch("/api/dsdesktop/balance/cost-store")
				.then((res) => res.json())
				.then((result) => {
					if (result && result.ok) costStore.hydrateDailyMonthly(result.daily, result.weekly, result.monthly);
				})
				.catch(() => {
					// 读不到 host 文件就继续用 localStorage，不阻塞界面。
				});
		}
		...
		/** 本日费用显示：已结算部分 + 本日进行中的实时估算。 */
		function formatDailyCost(cost, fallbackCurrency) {
			const liveCost = (Array.isArray(cost.live) ? cost.live : [])
				.filter((item) => item.startTime !== void 0 && dayKeyOf(item.startTime) === dayKeyOf(new Date()))
				.reduce((sum, item) => sum + (item.cost ?? 0), 0);
			const amount = (cost.daily?.totalCost ?? 0) + liveCost;
			const currency = cost.daily?.currency ?? cost.currency ?? fallbackCurrency ?? null;
			return currency === null ? formatMoney("", amount).trim() : formatMoney(currency, amount);
		}

		/** 周费用显示：已结算部分 + 本周进行中的实时估算。 */
		function formatWeeklyCost(cost, fallbackCurrency) {
			const liveCost = (Array.isArray(cost.live) ? cost.live : [])
				.filter((item) => item.startTime !== void 0 && weekKeyOf(item.startTime) === weekKeyOf(new Date()))
				.reduce((sum, item) => sum + (item.cost ?? 0), 0);
			const amount = (cost.weekly?.totalCost ?? 0) + liveCost;
			const currency = cost.weekly?.currency ?? cost.currency ?? fallbackCurrency ?? null;
			return currency === null ? formatMoney("", amount).trim() : formatMoney(currency, amount);
		}

		/** 月度费用显示：已结算部分 + 本月进行中的实时估算。 */
		function formatMonthlyCost(cost, fallbackCurrency) {
			const liveCost = (Array.isArray(cost.live) ? cost.live : [])
				.filter((item) => item.startTime !== void 0 && monthKeyOf(item.startTime) === monthKeyOf(new Date()))
				.reduce((sum, item) => sum + (item.cost ?? 0), 0);
			const amount = (cost.monthly?.totalCost ?? 0) + liveCost;
		 ...
		 */
		function MessageCostProbe({ sessionId, useSession, useChat, turn, seq, modelDirectories }) {
			const node = typeof useChat === "function"
				? useChat((snapshot) => findAssistantNode(snapshot, seq))
				: typeof useSession === "function"
					? useSession((snapshot) => findAssistantNode(snapshot, seq))
					: void 0;
	...
	function LiveCostProbe({ sessionId, useSession, useChat, modelDirectories }) {
		...
		/**
		 * 观察任意 session 的最终 assistant 节点，把精确 usage 写入 costStore。
		 * 背景工作区/会话完成后，turnTail 可能只挂在当前会话；这里从 session 快照
		 * 直接扫 nodes，保证本日/月度费用也能在后台会话结束时更新。
		 */
		function observeSessionFinal(sessionId, session, modelDirectories, feedVersion, isCurrent) {
			let snapshot;
			try {
			...
			const nodes = chatNodesOf(snapshot);
			...
			for (const node of nodes) {
				if (!node || node.kind !== "assistant" || node.usage === void 0 || node.usage === null) continue;
				const accountKey = `${sessionId}:${node.seq}`;
				const turnId = node.turn ?? node.turnId;
				const turnStartTime = snapshot?.turnTimings?.get?.(turnId)?.startTime ?? void 0;
				// 已按真实价格结算过的消息直接跳过，避免每次会话更新都重复扫描旧节点。
				if (turnStartTime === void 0 || costStore.isPricedAccounted(accountKey)) continue;
				getPricing().then((pricing) => {
					if (feedVersion !== void 0 && !isCurrent()) return;
					costStore.noteMessage(sessionId, accountKey, turnStartTime, node.usage, selection, pricing, turnId, node.step);
				});
			}
		}
			...
			getPricing().then((pricing) => {
				...
				const key = priceKey(selection);
				const priceEntry = pricing && selection ? pricing.modelPricing?.[key] : void 0;
				const peak = selection && isDeepSeekProvider(selection.provider) ? isPeakHours(billedAt) : false;
			...
			react.useEffect(() => {
				...
				for (const sessionId of ids) {
					...
					const feed = () => {
						...
						observeSessionFinal(sessionId, session, modelDirectories, current, () => version === current);
					};
					feed();
		 ...
		 *
		 * 「本次打开用量」和「用量汇总（日/周/月）」共用这一个组件——两处是同一套口径、
		 * 同一批列，只是统计窗口不同；各写一份表格的话，以后加一列就必然漏掉另一处。
		 *
		 * 行序跟随下方的价格表（configuredOrder 是「价格表里第几行」的索引），价格表里
		 * 没有配置单价的模型排到最后，保持两张表上下对读时模型顺序一致。
		 */
		function UsageTable({ t, models, configuredOrder, empty }) {
			...
			const rows = models.slice().sort((a, b) => {
		...
		function BalanceDetailsPanel({ t, store }) {
			...
			// 日/周/月三份用量跟费用汇总那三格是同一批累计器（daily/weekly/monthly），
			// 所以这张表跟下面的费用汇总天然对得上：同一个周期、同一批消息。
			...
			// 「本次打开」和当前周期都已经是 0 就没什么可清的，按钮置灰——不然点一遍
			// 二次确认，结果什么都没变。
			const hasResettable = periodUsage.length > 0 || Number(periodSource?.totalCost ?? 0) > 0
				|| cost.perModel.length > 0 || Number(cost.totalCost ?? 0) > 0;
			// 顶部展示的 API 厂家：优先单价表里出现的 provider，再并上实际用量里出现的 provider。
			const providerNames = [...new Set([
				...Object.keys(pricing?.modelPricing ?? {}).map((key) => key.split(":")[0]),
				...cost.perModel.map((m) => m.provider).filter((provider) => Boolean(provider))
			])].map(providerDisplayName).filter((name) => Boolean(name));
			const peakNow = isPeakHours(new Date());
			const hasDeepSeek = hasDeepSeekPricing(pricing);
				...
				react_jsx_runtime.jsxs("div", { className: "dsbPanelBody", children: [
					...
					react_jsx_runtime.jsxs("div", { className: "dsbSection", children: [
						...
						react_jsx_runtime.jsx(UsageTable, {
							t,
							models: cost.perModel,
							configuredOrder,
							empty: t("balance.usage.empty")
						})
					] }),
					react_jsx_runtime.jsxs("div", { className: "dsbSection", children: [
					...
					react_jsx_runtime.jsxs("div", { className: "dsbSection", children: [
						...
						react_jsx_runtime.jsxs("table", { className: "dsbCostTable", children: [
							react_jsx_runtime.jsxs("thead", { children: react_jsx_runtime.jsxs("tr", { children: [
								react_jsx_runtime.jsx("th", { children: t("balance.cost.title") }),
								react_jsx_runtime.jsx("th", { children: t("balance.daily.title") }),
								react_jsx_runtime.jsx("th", { children: t("balance.weekly.title") }),
								react_jsx_runtime.jsx("th", { children: t("balance.monthly.title") })
							] }) }),
							react_jsx_runtime.jsxs("tbody", { children: [
								react_jsx_runtime.jsxs("tr", { children: [
									...
									react_jsx_runtime.jsx("td", { children: formatWeeklyCost(cost, pricing?.currency ?? null) }),
						...
						configuredModels.length === 0
							...
							: react_jsx_runtime.jsxs("table", { className: "dsbPriceTable", children: [
								...
								react_jsx_runtime.jsxs("tbody", { children: configuredModels.map(([key, base]) => {
									...
									const rowMultiplier = isDeepSeekProvider(key.split(":")[0]) ? multiplier : 1;
									return react_jsx_runtime.jsxs("tr", { children: [
										react_jsx_runtime.jsx("td", { className: "dsbPriceModel", children: modelShortName(key) }),
										react_jsx_runtime.jsx("td", { className: "dsbPriceValue", children: `${formatUnitPrice(base.cacheHitPerMillion * rowMultiplier)} ${unit}` }),
										react_jsx_runtime.jsx("td", { className: "dsbPriceValue", children: `${formatUnitPrice(base.cacheMissPerMillion * rowMultiplier)} ${unit}` }),
										react_jsx_runtime.jsx("td", { className: "dsbPriceValue", children: `${formatUnitPrice(base.outputPerMillion * rowMultiplier)} ${unit}` })
									] }, key);
								}) })
							] })
					] })
