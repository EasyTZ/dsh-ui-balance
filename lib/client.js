window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = ""
			...
			+ ".dsbPanelBody .dsbSection > .dsbRow,.dsbPanelBody .dsbSection > .dsbNote,.dsbPanelBody .dsbSection > .dsbCostTable,.dsbPanelBody .dsbSection > .dsbPriceTable,.dsbPanelBody .dsbSection > .dsbPeriodTabs{margin-left:16px}"
		...
		/**
		 * 「本次打开花费」的统计下界。默认就是 appOpenTime；点了重置之后抬到当下，
		 * 所以它单独存一份，不跟 appOpenTime 混用——appOpenTime 还要用来判断一条
		 * 进行中的消息是不是这次启动之后才开始的。
		 */
		const SINCE_KEY = "dsh-ui-balance/spendSince/v1";
		let spendSince = Number(readSessionValue(SINCE_KEY)) || appOpenTime;
		writeSessionValue(SINCE_KEY, spendSince);
		...
		const EMPTY_PERIOD = Object.freeze({ key: null, currency: null, totalCost: 0, perModel: Object.freeze([]) });
		 ...
		 *
		 * 现在这些全都不存在了：日/周/月/本次打开四个数字都由 host 半从 dsh 的会话
		 * 事件日志现算（`usage-log.js`），那里每条消息都带 provider 报的精确 usage 和
		 * 逐条精确的 `{provider, model}`。浏览器半只剩两件事——把结果拉下来，以及为
		 * **正在生成中**的那条消息补一个字符数估算，让数字在回合结束前也会动。
		 */
		const costStore = (() => {
			// host 半算好的四格数字。live 之外的一切都以它为准。
			let served = { status: "loading", currency: null, session: EMPTY_PERIOD, daily: EMPTY_PERIOD, weekly: EMPTY_PERIOD, monthly: EMPTY_PERIOD };
			...
			function freeze() {
				return Object.freeze({
					...
					totalCost: served.session.totalCost,
					perModel: served.session.perModel,
					live: Object.freeze(
			...
			function refresh() {
				...
				inFlight = fetch(`/api/dsdesktop/balance/usage?since=${encodeURIComponent(String(spendSince))}`)
					.then((res) => res.json())
					.then((result) => {
						...
						served = {
							...
							session: normalizePeriod(result.session),
			...
			/**
			 * 延后刷一次。回合刚结束时用：那条消息要先由 host 半落进会话日志，我们才
			 * 读得到，立刻拉一次多半还是旧数字。
			 */
			...
			return {
				 ...
				 * 「本次打开」那一格同理，把本地的 spendSince 抬到当下。
				 *
				...
				resetPeriod(period) {
					...
					fetch("/api/dsdesktop/balance/reset", {
						...
						.catch(() => {
							// 写不进去就只有「本次打开」被清掉；下一次刷新会把周期数字原样拉回来。
						})
						.finally(() => refresh());
				}
			};
		 ...
		 *
		 * 拿不到 turnTimings 时 `startTime` 是 undefined——那条消息**正在生成**，
		 * 按定义就发生在今天/本周/本月，所以按「算进来」处理。反过来（要求
		 * startTime 存在才算）会让这几格在没有 turnTimings 的会话里一直不动，
		 * 只有「本次打开」那格会跳，看上去像是数字对不上。
		 */
