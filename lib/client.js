window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const costStore = (() => {
			...
			let live = null; // { turn, step, tokens:{...}, cost, provider, model, priced }
			...
			function liveChanged(next) {
				...
				return live.turn !== next.turn || live.step !== next.step || live.priced !== next.priced
			...
			function noteLiveEstimate({ turn, step, tokens, cost, selection, priced, currency: currencyCode }) {
			...
			function clearLive(turn, step) {
				if (live === null) return;
				if (turn !== void 0 && live.turn !== turn) return;
				if (step !== void 0 && live.step !== step) return;
				live = null;
				notify();
			}
			...
			return {
				...
				noteLiveEstimate,
				clearLive,
				noteMessage(accountKey, turnStartTime, usage, selection, pricing, turnId, step) {
					...
					if (live !== null && (turnId === void 0 || live.turn === turnId) && (step === void 0 || live.step === step)) {
						live = null;
					}
		...
		function MessageCostProbe({ sessionId, useSession, turn, seq, modelDirectories }) {
			...
			react.useEffect(() => {
				...
				getPricing().then((pricing) => {
					if (!alive) return;
					costStore.noteMessage(`${sessionId}:${seq}`, turn?.start?.time, node.usage, directoryState?.current ?? void 0, pricing, turn?.turn, node?.step);
				});
				return () => {
					alive = false;
				};
