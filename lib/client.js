window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *
		 * `accounted` 按 `sessionId:seq` 去重：同一条消息的 turnTail 只可能触发一次
		 * 上报（探针挂载时机就是那条消息刚收工那一刻），但热重载、Strict Mode 或
		 * 面板重新打开都可能让同一个探针重新跑一遍 effect，去重表保证不会重复计。
		 */
		const costStore = (() => {
			let totalCost = 0;
			let currency = null;
			...
			return {
				...
				noteMessage(accountKey, turnStartTime, usage, selection, pricing) {
					...
					if (accounted.has(accountKey)) return;
