 ...
 */
/**
 * 花费预估用的单价（每百万 token，货币走 `currency`）。DeepSeek 官方定价会变
 * （历史上降过价、还有非高峰时段折扣），这里给的默认值是标准时段价格，仅供
 * 预估——实际扣费以 DeepSeek 账单为准。用之前建议去官方定价页核对一遍：
 * https://api-docs.deepseek.com/quick_start/pricing
 * 写死会有两个问题：官方一调价面板就报错的数字；用户指到别的兼容网关，
 * 那边计价体系完全不同，这几个字段就是留给他们改的口子（同 baseURL 的理由）。
 */
export const Config = z.object({
  baseURL: z.string().default("https://api.deepseek.com"),
  currency: z.string().default("CNY"),
  priceCacheHitPerMillion: z.number().default(0.5),
  priceCacheMissPerMillion: z.number().default(2),
  priceOutputPerMillion: z.number().default(8)
});
...
async function handle(ctx, config, req, res) {
  ...
  const payload = {
    ...
    pricing: {
      ...
      cacheHitPerMillion: config.priceCacheHitPerMillion,
      cacheMissPerMillion: config.priceCacheMissPerMillion,
      outputPerMillion: config.priceOutputPerMillion
    }
  };
