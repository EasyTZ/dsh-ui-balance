...
export function parsePeakSchedule(html) {
  ...
  const sentences = text.split(/[。;；]/).map((raw) => raw.trim().replace(/^[（(]\d+[)）]\s*/, ""));
...
async function getPricingBundle(config) {
  ...
  const withOverrides = (pricing, fallbackSchedule) => ({
    ...mergeModelPricing(pricing, config),
    peakSchedule: effectivePeakSchedule(pricing?.peakSchedule ?? fallbackSchedule ?? null, config, calendar)
  });
  const timeline = pricingTimelineOf(entries, config).map((entry, index, all) => ({
    ...entry,
    ...
    pricing: withOverrides(entry.pricing, index === all.length - 1 ? (synced?.peakSchedule ?? null) : null)
  }));

  const current = timeline.length > 0
    ? (pricingAt(timeline, now) ?? timeline[timeline.length - 1].pricing)
    : withOverrides(synced !== null ? synced : pricingOf(config));
  ...
  return {
    ...
    status: {
      ...
      pendingFrom: timeline.filter((entry) => entry.from > now).map((entry) => entry.from)
    }
  };
}
